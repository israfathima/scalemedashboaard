"""MCA Lead Intelligence Platform: premium investor-grade analytics dashboard."""
...
def load_data() -> pd.DataFrame:
    ...
    scored["company_age_years"] = (
        ((pd.Timestamp(DATA_AS_OF) - scored["registration_date"]).dt.days / 365.25)
        .fillna(0)
        .clip(lower=0)
        .round(1)
    )
...
def filter_leads(
    ...
    filtered = filtered[
        filtered["company_age_years"].between(age_range[0], age_range[1])
    ]
    ...
    return [
        {
            ...
            "value": f"{total:,}",
            "note": "Current filtered universe",
        },
        {
            "title": "High Leads",
            "value": f"{high_leads:,}",
            "note": "Priority lead opportunities",
        },
...
def registration_trend_chart(frame: pd.DataFrame) -> go.Figure:
    timeseries = (
        frame.assign(
            month=frame["registration_date"].dt.to_period("M").dt.to_timestamp()
        )
...
def city_map(frame: pd.DataFrame) -> go.Figure:
    city_summary = frame.groupby("roc_city", as_index=False).agg(
        companies=("cin", "count"),
        high_leads=("lead_category", lambda x: (x == "High").sum()),
    )
    city_summary["coordinates"] = city_summary["roc_city"].map(get_city_coordinates)
    city_summary["lat"] = city_summary["coordinates"].map(lambda x: x[0])
    city_summary["lon"] = city_summary["coordinates"].map(lambda x: x[1])
    city_summary = city_summary.dropna(subset=["lat", "lon"]).nlargest(20, "companies")
    if city_summary.empty:
        return go.Figure()
    city_summary["lead_density"] = (
        city_summary["high_leads"] / city_summary["companies"]
    )
    fig = px.scatter_geo(
        city_summary,
        lat="lat",
    ...
    fig.update_layout(
        ...
        coloraxis_colorbar=dict(title="Lead density"),
    )
    return fig
...
def lead_quality_pie(frame: pd.DataFrame) -> go.Figure:
    quality_summary = (
        frame.groupby("lead_category", as_index=False)
        .agg(companies=("cin", "count"))
        .sort_values("companies", ascending=False)
    )
    fig = px.pie(
        ...
        names="lead_category",
        hole=0.58,
        color_discrete_sequence=["#22C1B5", "#0F172A", "#94A3B8"],
        height=420,
    )
    fig.update_traces(
        textinfo="percent+label",
...
def industry_ranking_bar(frame: pd.DataFrame) -> go.Figure:
    industry_summary = (
        frame.groupby("industry", as_index=False)
        .agg(companies=("cin", "count"))
        .sort_values("companies", ascending=True)
    ...
    fig = go.Figure(
        go.Bar(
            x=industry_summary["companies"],
            y=industry_summary["industry"],
            orientation="h",
            marker_color="#22C1B5",
            text=industry_summary["companies"].map("{:,}".format),
            textposition="inside",
        )
    )
    fig.update_layout(
        margin=dict(t=16, b=30, l=180, r=20),
        paper_bgcolor="#FFFFFF",
        plot_bgcolor="#FFFFFF",
        xaxis=dict(showgrid=False, zeroline=False),
        yaxis=dict(showgrid=False),
...
def capital_segment_rows(frame: pd.DataFrame) -> pd.DataFrame:
            ...
            [
                {
                    "segment": "Small",
                    "companies": 0,
                    "authorized_capital_lakh": 0,
                    "paid_up_capital_lakh": 0,
                },
                {
                    "segment": "Medium",
                    "companies": 0,
    ...
    summary = frame.groupby("segment", as_index=False).agg(
        companies=("cin", "count"),
        authorized_capital_lakh=("authorized_capital_lakh", "sum"),
        paid_up_capital_lakh=("paid_up_capital_lakh", "sum"),
    )
    summary["avg_capital_lakh"] = (
        summary["authorized_capital_lakh"] / summary["companies"]
    ).round(1)
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        st.markdown(
            "<div class='filter-heading'>Dashboard Filters</div>",
            unsafe_allow_html=True,
        )
        st.markdown(
            "<div class='filter-copy'>Refine results with location, industry, age, lead score and company search.</div>",
            unsafe_allow_html=True,
        )
    ...
    st.markdown(
        "<div class='hero-title'>MCA Lead Intelligence Platform</div>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<div class='hero-text'>Analyze Indian company registrations, identify high-value leads and explore market intelligence insights.</div>",
        unsafe_allow_html=True,
    )
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    st.markdown(
        "<span class='section-label'>Registration Trends</span>", unsafe_allow_html=True
    )
    st.markdown(
        "<div class='section-title'>Registration Trends</div>", unsafe_allow_html=True
    )
    st.markdown(
        "<div class='section-copy'>Smooth company registration insights with high-lead momentum over time.</div>",
        unsafe_allow_html=True,
    )
    st.markdown("</div>", unsafe_allow_html=True)
    st.markdown("<div class='chart-card'>", unsafe_allow_html=True)
    st.plotly_chart(
        registration_trend_chart(filtered),
    ...
    registration_mini = [
        {
            ...
            "note": "Strongest registration period",
        },
        {
            "title": "Total Registrations",
            "value": f"{len(filtered):,}",
            "note": "Filtered registrations",
        },
        {
            "title": "High Leads",
            "value": f"{int((filtered['lead_category'] == 'High').sum()):,}",
            "note": "Qualified leads",
        },
        {
            "title": "Active Industries",
            "value": f"{filtered['industry'].nunique():,}",
    ...
    st.markdown("<div class='section-header'>", unsafe_allow_html=True)
    st.markdown(
        "<span class='section-label'>Cities & Location Intelligence</span>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<div class='section-title'>City Clusters & Market Density</div>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<div class='section-copy'>Rank top cities and explore location intelligence with lead-quality bubbles.</div>",
        unsafe_allow_html=True,
    )
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    with col1:
        st.markdown("<div class='chart-card'>", unsafe_allow_html=True)
        st.markdown(
            "<div class='chart-title'>Top Registration Cities</div>",
            unsafe_allow_html=True,
        )
        st.markdown(
            "<div class='chart-copy'>Horizontal ranking view of the most active cities.</div>",
            unsafe_allow_html=True,
        )
        st.plotly_chart(
    ...
    with col2:
        st.markdown("<div class='chart-card'>", unsafe_allow_html=True)
        st.markdown(
            "<div class='chart-title'>India Location Intelligence</div>",
            unsafe_allow_html=True,
        )
        st.markdown(
            "<div class='chart-copy'>Bubble markers represent company count and lead quality density.</div>",
            unsafe_allow_html=True,
        )
        st.plotly_chart(
            city_map(filtered),
    ...
    city_insights = [
        {
            "title": "Fastest Growth City",
            "note": (
                filtered.groupby("roc_city")["cin"]
                .count()
                .pct_change()
                .fillna(0)
                .idxmax()
        ...
        },
        {
            ...
            "note": (
                filtered.groupby("roc_city")["authorized_capital_rs"].sum().idxmax()
                if not filtered.empty
                else "N/A"
            ),
        },
        {
            "title": "Best Lead Density",
            "note": (
                filtered.groupby("roc_city")["lead_score"].mean().idxmax()
                if not filtered.empty
                else "N/A"
            ),
        },
        {
            "title": "Recommended Expansion Market",
            "note": (
                filtered["roc_city"].mode().iloc[0] if not filtered.empty else "N/A"
            ),
        },
    ]
    ...
    st.markdown("<div class='section-header'>", unsafe_allow_html=True)
    st.markdown(
        "<span class='section-label'>Industry Intelligence</span>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<div class='section-title'>Sector Opportunity Snapshot</div>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<div class='section-copy'>Minimal, elegant industry insights for decision-ready analysis.</div>",
        unsafe_allow_html=True,
    )
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    st.markdown(
        f"<div class='chart-copy'>Selected theme: <strong>{selected_theme}</strong></div>",
        unsafe_allow_html=True,
    )
    ...
    with col1:
        st.markdown("<div class='chart-card'>", unsafe_allow_html=True)
        st.markdown(
            "<div class='chart-title'>Industry Composition</div>",
            unsafe_allow_html=True,
        )
        st.markdown(
            "<div class='chart-copy'>Top sectors by company count.</div>",
            unsafe_allow_html=True,
        )
        st.plotly_chart(
            industry_donut(filtered),
            use_container_width=True,
            config={"displayModeBar": False},
        )
        st.markdown("</div>", unsafe_allow_html=True)
    with col2:
        st.markdown("<div class='chart-card'>", unsafe_allow_html=True)
        st.markdown(
            "<div class='chart-title'>Top Industries</div>", unsafe_allow_html=True
        )
        st.markdown(
            "<div class='chart-copy'>Highest ranking industries from the filtered view.</div>",
            unsafe_allow_html=True,
        )
        st.plotly_chart(
            industry_ranking_bar(filtered),
            use_container_width=True,
            config={"displayModeBar": False},
        )
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    st.markdown("<div class='section-header'>", unsafe_allow_html=True)
    st.markdown(
        "<span class='section-label'>Lead Quality Breakdown</span>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<div class='section-title'>Lead Quality & Engagement</div>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<div class='section-copy'>A clear view of category mix, engagement and priority companies.</div>",
        unsafe_allow_html=True,
    )
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    with col1:
        st.markdown("<div class='chart-card'>", unsafe_allow_html=True)
        st.markdown(
            "<div class='chart-title'>Lead Category Mix</div>", unsafe_allow_html=True
        )
        st.markdown(
            "<div class='chart-copy'>High / Medium / Low distribution.</div>",
            unsafe_allow_html=True,
        )
        st.plotly_chart(
            lead_quality_pie(filtered),
            use_container_width=True,
            config={"displayModeBar": False},
        )
        st.markdown("</div>", unsafe_allow_html=True)
    with col2:
        st.markdown("<div class='chart-card'>", unsafe_allow_html=True)
        st.markdown(
            "<div class='chart-title'>Industry Engagement</div>", unsafe_allow_html=True
        )
        st.markdown(
            "<div class='chart-copy'>Average lead score by industry.</div>",
            unsafe_allow_html=True,
        )
        st.plotly_chart(
            engagement_bars(filtered),
            use_container_width=True,
            config={"displayModeBar": False},
        )
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    st.markdown(
        "<div class='section-title'>Priority Companies</div>", unsafe_allow_html=True
    )
    st.markdown(
        "<div class='section-copy'>Top priority companies with lead score and status.</div>",
        unsafe_allow_html=True,
    )
    st.markdown("</div>", unsafe_allow_html=True)

    top_leads = filtered.nlargest(5, "lead_score")[
    ...
    st.markdown("<div class='section-header'>", unsafe_allow_html=True)
    st.markdown(
        "<span class='section-label'>Capital Analysis</span>", unsafe_allow_html=True
    )
    st.markdown(
        "<div class='section-title'>Capital Allocation by Company Size</div>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<div class='section-copy'>Authorized and paid capital for small, medium and large companies.</div>",
        unsafe_allow_html=True,
    )
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    st.markdown("<div class='chart-card'>", unsafe_allow_html=True)
    st.plotly_chart(
        capital_segment_chart(capital_summary),
        use_container_width=True,
        config={"displayModeBar": False},
    )
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    capital_mini = [
        {
            ...
            "value": format_inr_crore(filtered["authorized_capital_rs"].sum()),
            "note": "Current filtered capital",
        },
        {
            "title": "Paid Capital",
            "value": format_inr_crore(filtered["paid_up_capital_rs"].sum()),
            "note": "Paid-up amount",
        },
        {
            "title": "Avg Lead Score",
            "value": f"{filtered['lead_score'].mean():.1f}%",
            "note": "Quality average",
        },
        {
            "title": "Largest Capital Company",
            "value": (
                filtered.sort_values("authorized_capital_rs", ascending=False)[
                    "company_name"
                ].iloc[0]
                if not filtered.empty
                else "N/A"
            ),
            "note": "Top capital firm",
        },
    ]
    ...
    st.markdown("<div class='section-header'>", unsafe_allow_html=True)
    st.markdown(
        "<span class='section-label'>Search & Company Explorer</span>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<div class='section-title'>Search, Explore and Export</div>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<div class='section-copy'>Search companies, preview details, paginate results and export the filtered universe.</div>",
        unsafe_allow_html=True,
    )
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    st.markdown(
        "<div class='table-title'>Company Explorer</div>", unsafe_allow_html=True
    )
    st.markdown(
        f"<div class='table-copy'>Showing page {page} of {total_pages} • {total_rows:,} companies in the current filter.</div>",
        unsafe_allow_html=True,
    )
    explorer_view = paged.rename(
        columns={
    ...
    if not paged.empty:
        ...
        st.markdown(
            "<div class='final-card-title'>Company Details Preview</div>",
            unsafe_allow_html=True,
        )
        st.markdown(
    ...
    st.markdown("<div class='section-header'>", unsafe_allow_html=True)
    st.markdown(
        "<span class='section-label'>Final Insights</span>", unsafe_allow_html=True
    )
    st.markdown(
    ...
    st.markdown(
        "<div class='section-copy'>Four simple executive insights from the current view.</div>",
        unsafe_allow_html=True,
    )
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    final_insights = [
        {
            "title": "Best Expansion Region",
            "note": filtered["state"].mode().iloc[0] if not filtered.empty else "N/A",
        },
        {
            "title": "Highest Opportunity Sector",
            "note": (
                filtered.groupby("industry")["lead_score"].mean().idxmax()
                if not filtered.empty
                else "N/A"
            ),
        },
        {
            ...
            "note": (
                filtered.groupby("roc_city")["cin"].count().idxmax()
                if not filtered.empty
                else "N/A"
            ),
        },
